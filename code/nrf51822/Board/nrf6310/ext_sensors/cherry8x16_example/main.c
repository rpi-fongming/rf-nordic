...
/** @file
 * @brief Cherry 8x16 keyboard example.
 * @defgroup cherry8x16_example Cherry 8x16 keyboard example
 * @{
 * @ingroup nrf_examples_nrf6310
 *
 * @brief Cherry 8x16 keyboard example.
 *
 * This example will detect the characters by scanning the matrix for any state change (key(s) press or release).
 * If any changes from the previous state is identified, then the received key codes are converted from hid to
 * ascii which are transmitted over uart and can be listened by any COM listener like example:PUTTY (this is
 * default behavior). However commenting out USE_UART can be done to to output modifier key presses to LEDs
 * through GPIOs.
 *
 * This example is meant to be used with the nRF6423 board. Connect nRF6310 P1 to LEDs.
 ...
 * - (rows)          P2   -   P2
 *
 * Can also be used with other keyboards, in which case row and column mapping have to be changed in this example
 */
 ...
 * so comment out UART code if using LEDs for modifier keys and vice-versa */
 ...
 * Pointer to GPIO memory are to use as a row port. No pointer alignment required for pointing to 8-bit values in this memory region.
 */
static const uint8_t volatile * const matrix_row_port = ((uint8_t*)(&NRF_GPIO->IN)) + 2;

/** @var static uint16_t * const matrix_column_port
 * Pointer to GPIO memory are to use as a column port. Half-word alignment is required for pointing to 16-bit values in this memory region.
 */
...
/** Sends a series of characters (string) to UART.
