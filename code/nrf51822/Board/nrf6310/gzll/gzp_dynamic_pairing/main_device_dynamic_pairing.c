 ...
 *
 * 1) nRF24Lxx Host running the gzll_device_w_dynamic_pairing example from the 
 * compatible version of the nRFgo SDK, or a
 *
 * 2) nRF51 Host running the gzp_device_dynamic_pairing_example example.
 * 
 *
...
int main(void)
{
  ...
  init_ok = init_ok && nrf_gzll_set_max_tx_attempts(200);
  init_ok = init_ok && nrf_gzll_set_timeslot_period(NRF_GZLLDE_RXPERIOD_DIV_2);  // Half RX period on nRF24Lxx device
  ...
  if(init_ok)
  {
      ...
      {      
        ...
        else
        {
          ...
          {
            // Initialization of Gazell failed. 
