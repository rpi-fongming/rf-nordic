...
/**@brief Maximum iterations needed in the main loop between stop bit 1st byte and start bit 2nd
 * byte. DTM standard allows 5000us delay between stop bit 1st byte and start bit 2nd byte. Byte
 * transmission time is 10 * 1/19200 = approximately 5200 us/byte (including start/stop). The max
 * time between two bytes is 10200us/625us/iteration =  17.3 iterations. This is rounded up to 18.
 * If UART bit rate is changed, this value should be recalculated as well
 */
#define MAX_ITERATIONS_NEEDED_FOR_NEXT_BYTE 18
...
/**@brief Configuration of pin to use for UART TX.
 *
...
/**@brief Configuration of pin to use for UART RX.
 *
...
int main(void)
{
    ...
    uint8_t      byteRx;                          // Last byte read from UART.
    ...
    for (;;)
    {
        ...
        NRF_UART0->EVENTS_RXDRDY = 0;
        byteRx = (uint8_t)NRF_UART0->RXD;

        if (!is_msb_read)
        {
            // This is first byte of two-byte command.
            is_msb_read       = true;
            dtm_cmd_from_uart = ((dtm_cmd_t)byteRx) << 8;
        ...
        is_msb_read        = false;
        dtm_cmd_from_uart |= (dtm_cmd_t)byteRx;
